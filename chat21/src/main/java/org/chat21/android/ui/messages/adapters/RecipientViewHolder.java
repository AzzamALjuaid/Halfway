...
class RecipientViewHolder extends RecyclerView.ViewHolder {
    ...
    void bind(final Message previousMessage, final Message message,
        ...
        if (message.getType().equals(Message.TYPE_IMAGE)) {
        ...
        }else if (message.getType().equals(Message.TYPE_LOCATION)) {
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (ChatUI.getInstance().getOnLocationMessageClickListener() != null) {
                        String[] split = message.getText().split(",");
                        ChatUI.getInstance().getOnLocationMessageClickListener().onMessageClicked(
                                new LocationMessage(
            ...
            mPreview.setVisibility(View.VISIBLE);
            mMessage.setVisibility(View.GONE);
            mPreview.setImageResource(R.drawable.ic_baseline_map_24);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                mPreview.setImageTintList(ColorStateList.valueOf(itemView.getContext().getColor(R.color.colorAccent)));

            }
            setLocationAction(message);
    ...
    private void setLocationAction(Message message) {
    ...
    private void setPreview(final Message message) {
        ...
        Glide.with(itemView.getContext())
                .load(getImageUrl(message)).addListener(new RequestListener<Drawable>() {
            @Override
